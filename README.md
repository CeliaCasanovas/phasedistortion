...
You can freely define your 5-segment transfer function. You can only read from a one-shot sine table, but if you set one of the transfer function's y values to 1 you'll get to the end of the sine for that segment and the table wraps properly for the next segment, so you can kinda do it. Maybe I'll try and implement running through the table twice at double the increment, but I don't think there'll ever be more than 1 readout table because that'd cause massive cache thrashing. For a monophonic web port it probably doesn't matter but I want to make the C# version multitimbral and I'm kinda striving to make the DSP fast.
...
add morph the way it was on cz synths: interpolating between the transfer function and y = x, not between the target wave and a sine
add decoupled mixer logic
add ring modulation
refine the frequency parameter into octave, coarse and fine
add key follow modulation
are the envelopes ok? i want to tweak the curve ranges on the ui, but i mean the envelope model itself
add the 5-stage transfer function sequence and maybe a plockable control that lets you move forwards and backwards along the sequence
add chromatic playability lmao
add midi
add patch saving/patch management
